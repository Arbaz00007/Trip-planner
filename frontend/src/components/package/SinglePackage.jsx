    ...
    <div className="singleProduct flex h-[100vh] p-4">
      <div className="flex flex-col justify-between w-[50%] h-full p-6 relative">
        ...
        <div className="h-[80%] w-full flex flex-col justify-center items-center">
          ...
          <div className="h-[20%] w-[70%] flex justify-between gap-8">
            {imageArr?.map((data, inx) => (
              <div
                key={inx}
                className="border-[2.4px] border-black rounded-lg overflow-hidden h-full w-1/3 cursor-pointer"
        ...
        <div className="flex mb-8 px-12 items-start border-2 justify-between font-heading text-lg">
          <span>
            <span className="text-primary font-bold">Post by:</span>{" "}
            {products?.name}
          </span>
          <span>
      ...
      <div className="singlePro flex w-[50%] gap-4 p-6 flex-col items-center font-heading overflow-y-scroll">
        ...
        <div className="flex text-xl gap-1 font-bold justify-start items-center">
          ...
          <span className="text-gray-500 font-bold">{products?.location}</span>
        </div>

        <p className="text-justify text-lg p-4">{products.description}</p>
        <div className="flex gap-8">
          <span className="text-3xl">
            <span className="text-primary font-bold">Price: </span>Rs.{" "}
        ...
        <div className="flex justify-between w-full gap-4 font-bold">
          <button className="border-2 border-primary p-2 px-4 rounded-full my-grid">
              ...
              <form action="" className="bookingDateForm flex gap-8 ">
                ...
                {currentUser ? (
                  products.isBooked === "Booked" ? (
                    <>
                      <span className="bg-primary p-2 px-4 rounded-lg">
                        This package is booked
                      </span>
                    </>
                  ) : selectedDate ? (
                    <Link
                    ...
                    >
                      <button className="bg-primary p-2 px-4 rounded-lg">
                        Book
                      </button>
                    </Link>
                  ) : (
                    <button
                      ...
                      onClick={() => toast.error("Please select the date")}
                    >
                      Book
                    </button>
                  )
        ...
        <div className="w-full mx-auto p-6 bg-white shadow-md rounded-lg">
          ...
          <div className="flex items-start space-x-4 mb-4">
            ...
            <div>
              <p className="font-semibold">You'll get picked up</p>
          ...
          {itinerary.map((stop) => (
            <div
              key={stop.id}
              className="flex items-start space-x-4 mb-4 relative"
            >
              <BsCircleFill className=" p-2 bg-black text-white rounded-full" />
              <div>
          ...
          <div className="flex items-start space-x-4 mt-4">
            <FaMapMarkerAlt className="text-xl" />
            <p className="font-semibold">You'll return to the starting point</p>
          </div>
        </div>
